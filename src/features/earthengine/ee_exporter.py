...
def get_ee_credentials():
    gcp_sa_key_path = os.environ.get("GCP_SA_KEY")
    if gcp_sa_key_path is not None:
        # Read the JSON content from the file path
        with open(gcp_sa_key_path) as f:
            gcp_sa_key = f.read()
...
class EarthEngineExporter:
    ...
    def __init__(
        ...
        if mode == "url":
            print(
                f"Mode: url. Files will be saved to {self.local_tifs_folder} and rsynced to googlecloud"
            )
